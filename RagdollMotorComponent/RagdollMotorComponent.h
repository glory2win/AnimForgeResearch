// RagdollMotorComponent.h
//
// Custom joint-motor ragdoll controller -- the constraint-native replacement for the
// PhysicalAnimationComponent pipeline (and the RagdollStabilityComponent that babysat it).
//
...
//
// Why this behaves better than Physical Animation for the pileup/settle problems we hit:
...
//     safety net, same caveats as before).
...
#include "RagdollMotorComponent.generated.h"
...
UENUM(BlueprintType)
enum class ERagdollMotorState : uint8
{
	Inactive,
	BlendingIn,
	Active,
...
struct FRagdollMotorBoneProfile
{
	GENERATED_BODY()
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnRagdollMotorStateChanged, ERagdollMotorState, NewState);
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FOnRagdollMotorSettled);
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FOnRagdollMotorFinished);

// Replace YOURGAME_API with your project's generated API macro (e.g. MYGAME_API).
UCLASS(ClassGroup = (Physics), meta = (BlueprintSpawnableComponent))
class YOURGAME_API URagdollMotorComponent : public UActorComponent
{
	GENERATED_BODY()

public:
	URagdollMotorComponent();
	...
	UFUNCTION(BlueprintPure, Category = "Ragdoll")
	ERagdollMotorState GetState() const { return State; }

	UFUNCTION(BlueprintPure, Category = "Ragdoll")
	bool IsHitReactionActive() const { return bHitReaction && State != ERagdollMotorState::Inactive && State != ERagdollMotorState::Done; }
	...
	FOnRagdollMotorStateChanged OnStateChanged;
...
protected:
	...
	UPROPERTY(EditDefaultsOnly, Category = "Ragdoll|Motors")
	...
	UPROPERTY(EditDefaultsOnly, Category = "Ragdoll|Motors", meta = (ClampMin = "0.0"))
	...
	UPROPERTY(EditDefaultsOnly, Category = "Ragdoll|Motors")
	TArray<FRagdollMotorBoneProfile> BoneProfiles;
	...
	UPROPERTY(EditDefaultsOnly, Category = "Ragdoll|Motors")
	UCurveFloat* StrengthOverLifetimeCurve = nullptr;
	...
	UPROPERTY(EditDefaultsOnly, Category = "Ragdoll|Contact", meta = (ClampMin = "0.0", ClampMax = "1.0"))
	...
	UPROPERTY(EditDefaultsOnly, Category = "Ragdoll|HitReaction")
	...
	UPROPERTY(EditDefaultsOnly, Category = "Ragdoll|HitReaction", meta = (ClampMin = "0.0"))
	...
	UPROPERTY(EditDefaultsOnly, Category = "Ragdoll|OwnerFollow")
	...
	UPROPERTY(EditAnywhere, Category = "Ragdoll|Debug")
...
private:
	...
	ERagdollMotorState State = ERagdollMotorState::Inactive;
	...
	void SetState(ERagdollMotorState NewState);
